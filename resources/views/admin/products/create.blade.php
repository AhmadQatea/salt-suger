...
@section('title', 'إضافة صنف — ' . config('app.name'))
...
@section('content')
    <div class="admin-card">
        <div class="page-toolbar">
            <div>
                <h1>إضافة صنف</h1>
                <p class="subtitle">إنشاء صنف جديد للقائمة</p>
            </div>
            <a href="{{ route('admin.products.index') }}" class="btn-ghost">رجوع</a>
        </div>

        <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="admin-form" novalidate>
            @csrf
